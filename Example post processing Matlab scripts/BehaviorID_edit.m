...
Qedges = {};
for i = SEGMENTS
    Q{i} = (dAngles(i,:)<0.001);        %find where diff(angles) is <0.001
...
for j = SEGMENTS
    ...
    for i = 1:length(QboutsF2{j})
        if diff(QboutsF2{j}(i,:)) > frameRate*2        %Length Filter for Segment Qbouts (2 sec)
...
for j = 1:2
    ...
    for i = 1:size(QboutAll_t{j}(:,:),1)
        if diff(QboutAll_t{j}(i,:)) > frameRate*2        %Length Filter for Qbouts
...
for i = 1:length(Waves1)
    if ~isempty(Waves1{i}.FRWI)
        for j = 1:length(Waves1{i}.FRWI(:,1))
            ...
            Waves1{i}.FRWV(j) = (sum(diff(Waves1{i}.S(Range(1):Range(2))))*5) / ((Waves1{i}.T(Range(2))-Waves1{i}.T(Range(1)))/frameRate);
    ...
    if ~isempty(Waves1{i}.REVI)
        for j = 1:length(Waves1{i}.REVI(:,1))
            ...
            Waves1{i}.REVV(j) = -1*(sum(diff(Waves1{i}.S(Range(1):Range(2))))*5) / ((Waves1{i}.T(Range(2))-Waves1{i}.T(Range(1)))/frameRate);
...
for i = 1:length(Waves2)
    if ~isempty(Waves2{i}.FRWI)
        for j = 1:length(Waves2{i}.FRWI(:,1))
            ...
            Waves2{i}.FRWV(j) = (sum(diff(Waves2{i}.S(Range(1):Range(2))))*5) / ((Waves2{i}.T(Range(2))-Waves2{i}.T(Range(1)))/frameRate);
    ...
    if ~isempty(Waves2{i}.REVI)
        for j = 1:length(Waves2{i}.REVI(:,1))
            ...
            Waves2{i}.REVV(j) = -1*(sum(diff(Waves2{i}.S(Range(1):Range(2))))*5) / ((Waves2{i}.T(Range(2))-Waves2{i}.T(Range(1)))/frameRate);
...
%%
save([Worm ' Behavior'],'SlidWindow','frameRate','scrsz','Worm','SegNum','NumFrames','SEGMENTS','frames','AHA_S05','RT','RT_L',...
    ...
    'QBnextMB','MBnextQB','QBoutDur','QBoutNum',...
    'Waves1','Waves2','Waves_Pointer','F','R','D','Q','Unc','Behavior','WormLengths');
